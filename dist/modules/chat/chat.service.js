...
class ChatService {
    ...
    createChattingGroup = async (req, res) => {
        ...
        if (!users) {
            throw new error_response_1.NotFoundRequestException("No users found");
        }
        if (participants.length != users.length) {
            throw new error_response_1.NotFoundRequestException("some or all recipient all invalid");
        }
    ...
    sendMessage = async ({ content, socket, sendTo, io }) => {
        try {
            ...
            if (!user) {
                throw new error_response_1.NotFoundRequestException("invalid recipient friend");
            }
    ...
    joinRoom = async ({ roomId, socket, io }) => {
        try {
            ...
            if (!chat) {
                throw new error_response_1.NotFoundRequestException("fail to find matching room ");
            }
